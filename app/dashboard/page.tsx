...
export default function Dashboard() {
  ...
  useEffect(() => {
    ...
    if (shouldRefreshFromRoommates || shouldRefreshFromEdit || dashboardNeedsRefresh || hasRefreshParam) {
      ...
      if (navigateToWeek) {
        ...
        sessionStorage.removeItem('navigateToWeek');
      }
  ...
  useEffect(() => {
    ...
    const unsubscribeSchedule = events.on('schedule-updated', (event) => {
      console.log('[Dashboard] Schedule update event received:', event.detail)
    ...
    const unsubscribeWeek = events.on('week-changed', (event) => {
      console.log('[Dashboard] Week change event received:', event.detail)
      if (event.detail.source !== 'dashboard' && event.detail.weekDate) {
        setCurrentWeek(event.detail.weekDate)
      }
    })
    
    // Listen for sync requests
    const unsubscribeSync = events.on('sync-required', () => {
      console.log('[Dashboard] Sync request received')
      loadData()
    })
  ...
  const loadData = useCallback(async () => {
    ...
    try {
      ...
      if (!currentWeek || isNaN(currentWeek.getTime())) {
        console.error('Invalid currentWeek date')
        setCurrentWeek(new Date())
        return
      }
      ...
      if (usersError) {
        ...
        const typedUsers = usersData.map(user => ({
          ...
          initial: String(user.initial || user.name?.charAt(0) || '?')
        }))
        setUsers(typedUsers)
        ...
        // Fetch schedules for the current week using the new week-aware function
        console.log('[Dashboard] Loading schedules for week:', formatWeekRange(currentWeek))
  ...
  }, [isClient, userName, toast]); // Remove currentWeek to prevent infinite loop
  ...
  // Wrapper for formatWeekRange to handle server-side rendering
  ...
  const previousWeek = async () => {
    ...
    emitWeekChange(prevWeek, 'dashboard')
    ...
    setTimeout(() => setWeekLoading(false), 500)
  }
  ...
  const nextWeek = () => {
    ...
    emitWeekChange(nextWeek, 'dashboard')
  ...
  // Removed repeatPreviousWeek function - schedules should be entered manually
    ...
    <div className="flex flex-col min-h-screen bg-[#282828] text-white">
      ...
      <main className="flex-1 px-4 pb-20 pt-10 max-w-7xl mx-auto w-full relative" data-component-name="Dashboard">
        ...
        <div className="flex flex-col relative">
          ...
          {weekLoading && (
            <div className="absolute inset-0 bg-[#282828]/80 z-10 flex items-center justify-center rounded-lg">
              <div className="bg-[#333333] px-4 py-2 rounded-lg shadow-lg">
                <p className="text-sm text-[#A0A0A0]">Loading week data...</p>
              </div>
            </div>
          )}
      ...
      {isClient && userName && (
        ...
        >
          ...
          >
            <Link 
              href={`/schedule/edit?from=%2Fdashboard&day=${encodeURIComponent(getCurrentDay())}&week=${currentWeek.toISOString().split('T')[0]}`} 
              data-component-name="LinkComponent"
            >
